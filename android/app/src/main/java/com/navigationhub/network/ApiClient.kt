...
class ApiClient(private val context: Context) {
    ...
    private suspend fun execute(request: Request): String = withContext(Dispatchers.IO) {
        ...
        if (!response.isSuccessful) {
            val detail = try {
                JSONObject(body).optString("detail", body.substring(0, minOf(body.length, 200)))
    ...
    suspend fun registerDevice(name: String, deviceType: String = "android"): TokenResponse {
    ...
    suspend fun verifyPairing(code: String): PairingVerifyResponse {
        ...
        return PairingVerifyResponse(
            ...
            deviceId = obj.optString("device_id", null),
            apiToken = obj.optString("api_token", null)
        )
    }
    ...
    suspend fun pushNotification(request: PushNotificationRequest) {
        val json = JSONObject().apply {
            ...
            put("app_package", request.appPackage)
            put("app_name", request.appName)
            put("notification_type", request.notificationType)
            put("is_sms", request.isSms)
        ...
        try {
            ...
            Log.w(TAG, "Push notification failed: ${e.message}")
        }
    }
    ...
    suspend fun getDevices(): List<DeviceResponse> {
        ...
        return (0 until arr.length()).map { i ->
            ...
            DeviceResponse(
                ...
                deviceType = d.optString("device_type"),
                platform = d.optString("platform"),
                platformVersion = d.optString("platform_version"),
                lastSeen = d.optString("last_seen", null),
                isActive = d.optBoolean("is_active", true)
            )
        }
    }
